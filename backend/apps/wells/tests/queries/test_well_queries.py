...
class TestWellQueriesAnnotations:
    ...
    def test_cumulative_volume_is_null_when_table_absent(self, annotation_attr):
        """When production_monthly does not exist the annotation degrades to null."""
        WellHeaderFactory(base_uwi="CV-1")
        well = WellQueries.get_annotated_queryset().get(base_uwi="CV-1")
        # In the test database the table IS created (managed=True via conftest),
        # but no rows exist for this well, so the subquery returns null.
...
class TestProductionQueriesDailyProduction:
    ...
    def test_returns_empty_list_for_well_with_no_rows(self):
        ...
        # The table exists in the test DB (managed=True via conftest) but no
        # rows are inserted for this UWI.
    ...
    def test_null_values_coerced_to_zero(self, null_col, expected_value, db):
        ...
        assert rows[0][null_col.replace("daily_", "daily_")] == expected_value
